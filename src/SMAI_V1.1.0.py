...
Version Release Date : 2/29/2026
Filename : SMAI_v1.1.0.py
Version : v1.1.0
...
# Imports the os module for file path checking and reading and writing the config.txt file
...
# Function to load the API key from the config.txt file if it exists, if not it asks the user
# to enter their API key and saves it to a config.txt file for future use so the user
# only has to enter their key once and the program remembers it for all future sessions
def load_api_key():

    # Checks if the config.txt file already exists with a saved API key
    if os.path.exists("config.txt"):
        
        # Opens and reads the saved API key from the config.txt file and strips any whitespace
        with open("config.txt", "r") as f:
            return f.read().strip()
    
    # If config.txt does not exist, asks the user to enter their API key for the first time
    else:
        key = input("\nPlease enter your Alpha Vantage API key: ").strip()
        
        # Saves the entered API key to a new config.txt file for future use
        with open("config.txt", "w") as f:
            f.write(key)
        print("\n✅ API key saved to config.txt for future use.")
        return key
