...
# Imports the os module for file path checking and reading and writing the API keys text files
# Imports the groq module for AI implementation 
...
# Reconfigures the standard output to use UTF-8 encoding to allow emojis 
# and special characters to display correctly in terminals that default 
# to Windows cp1252 encoding (such as the VSCode terminal)
...
# to enter their API key and saves it to the specified file for future use so the user
# only has to enter their key once and the program remembers it for all future sessions
# Takes the filename and service name as arguments so the function can be reused for
# multiple API keys across different services
def load_api_key(filename, service_name):
    ...
    # If the file does not exist, asks the user to enter their API key for the first time
        ...
        key = input(f"\nPlease enter your {service_name} API key: ").strip()
        
        # Saves the entered API key to the specified file for future use
        with open(filename, "w") as f:
            f.write(key)
        print(f"\n✅ {service_name} API key saved to {filename} for future use.")
        return key

# Calls the load_api_key function to either load the saved key from config.txt or prompt
# the user to enter one for the first time and stores it in the apikey variable
apikey = load_api_key("AlphaVantageAPIKey.txt", "Alpha Vantage")
groqapikey = load_api_key("GroqAPIKey.txt", "Groq")
...
# Defines a function to get AI powered investment advice from the Groq API using the stock overview data
# Takes the overview dictionary as an argument and sends a formatted
# prompt to the Groq LLM which returns a detailed investment analysis
def ai_investment_advice(overview, current_price):
    
    # Creates the Groq client using the loaded Groq API key
    client = Groq(api_key=groqapikey)

    # Builds the prompt using the stock overview data
    prompt =f"""
    Company : {overview["Name"]}
    Current Stock Price : ${current_price:.2f}
    Sector: {overview['Sector'].title()}
    Industry: {overview['Industry'].title()}
    Beta: {overview['Beta']}
    Analyst Strong Buy Ratings: {overview['Analyst Rating Strong Buy']}
    Analyst Buy Ratings: {overview['Analyst Rating Buy']}
    Analyst Hold Ratings: {overview['Analyst Rating Hold']}
    Analyst Sell Ratings: {overview['Analyst Rating Sell']}
    Analyst Strong Sell Ratings: {overview['Analyst Rating Strong Sell']}
    """

    # Sends the prompt to Groq and stores the response
    response = client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        messages=[
            # System prompt tells Groq how to behave and what role to take
            {"role": "system", "content": system_promptGROQ},
            # User prompt contains the actual stock data
            {"role": "user", "content": prompt}
        ]
    )

    # Returns the AI generated investment advice text to the main program
...
print(" • Receive investment advice.")
print(" • Receive AI powered investment advice.")
print("\n=======================================================================")
...
# -------------------------------------------
    ...
    while True:
        ...
        print("4. Get AI powered investment advice.")
        print("5. Change the stock you'd like to view.")
        print("q. To quit.")
...
# -------------------------------------------
#
# AI Investment Advice + AI Questions (User Choice 4)
#
# -------------------------------------------   

        # If the user chooses (4): to get AI powered investment advice
        elif userchoices == "4":
            
            # Gets the stored stock overview data
            stock_data = overview_data

            # Gets the most recent daily close price to pass to the AI function
            dailydata = timeseriesdaily(userstockchoice)
            current_price = dailydata[0]["Close"]

            # AI Advice loading icon and text
            print("\n⏳ Generating AI analysis please wait...")

            # Gets the AI powered advice from the ai_investment_advice function
            ai_advice = ai_investment_advice(stock_data,current_price)

            # Prints the AI investment advice header
            print("\n-----------------------------------------------------------------------")
            print(f"\n🤖 AI Investment Advice for {companynamechecktest}:")
            print("\n-----------------------------------------------------------------------")

            # Prints the AI generated investment advice
            print(ai_advice)

            # Starts a while loop to allow the user to ask follow up questions
            while True:

                # Asks the user if they want to ask a follow up question or follow up questions
                print("\n-----------------------------------------------------------------------")
                followup = input("\nWould you like to ask a follow up question about this stock?\nPress (1) for YES or press (2) to return to the main menu: ").strip()

                # If the user wants to ask a follow up question
                if followup == "1":

                    # Asks the user to enter their question    
                    user_question = input("\nEnter your question: ").strip()

                    # AI Advice loading icon and text
                    print("\n⏳ Generating AI analysis please wait...")

                    # Sends the follow up question to Groq with the stock data as context
                    client = Groq(api_key=groqapikey)
                    ...
                    followup_response = client.chat.completions.create(
                        ...
                        messages=[
                            {"role": "system", "content": system_promptGROQ},
                            {"role": "user", "content": f"Stock Data: Company: {stock_data['Name']}, Current Price: ${current_price:.2f}, Sector: {stock_data['Sector']}, Industry: {stock_data['Industry']}, Beta: {stock_data['Beta']}. Question: {user_question}"}     
                        ]
                    )

                    # Prints the follow up response
                    print("\n-----------------------------------------------------------------------")
                    print(f"\n🤖 AI Response:")
                    print("\n-----------------------------------------------------------------------")
...
# -------------------------------------------
#
# Changing Stock (User Choice 5)
#
# -------------------------------------------   
        
        # If the user chooses (5): to change their stock choice
        elif userchoices == "5":
...
# -------------------------------------------     
        ...
        # If the users choice is invalid and not 1,2,3,4, or q prints out an invalid input message
            ...
            print("Sorry your input was invalid, please enter 1,2,3,4,5, or q to quit.")
